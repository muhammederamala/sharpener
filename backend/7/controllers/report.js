...
const { sequelize } = require('../util/database'); // Import your Sequelize instance
...
async function getYearlyReport(userId) {
    const expenses = await Expense.findAll({
        ...
        attributes: ['name', 'category', 'amount', 'createdAt'], // Adjust these attributes based on your Expense model
    });
...
async function getMonthlyReport(userId) {
    const expenses = await Expense.findAll({
        ...
        attributes: ['name', 'category', 'amount', 'createdAt'],
    });
